...
**What was surprising.**
- Efficiency holds far better here than in either other MPI kernel in this repo — 71.44% at 8 ranks, versus reduction's 20.26% at 8 ranks and matmul's 15.92% at 9. The difference is structural, not incidental: matvec communicates *once* (the initial scatter) and then runs fully independently, while Cannon's algorithm re-communicates every round and reduction re-combines every call.
- No sharp hyperthreading cliff at 8 ranks (71.44%, barely below 6 ranks' 71.93%) — unlike matmul's steep drop at the same worker counts. Consistent with matvec having no tight communication rounds for oversubscribed ranks to collide over; the workload is close to embarrassingly parallel once the one-time scatter is done.
