...
**What was surprising.**
- Both precisions land right around **104% of peak** — the same small overshoot pattern and magnitude as reduction's `reduce_shuffle`, and likely the same cause: `bandwidthTest`'s reference figure itself varies under GPU Boost, so a well-tuned kernel run can measure slightly above a peak captured in an earlier, separate run. That's a different (and much smaller) story than jacobi's >200% overshoot, which has no L2-cache-reuse explanation available here — SAXPY touches each element exactly once, with no neighbor overlap to be cached.
- Going in, I expected something more like 70–85% of peak for this warm-up kernel, and flagged ~20% as the signal something was badly misconfigured. Landing at 103–104% — comfortably above even the optimistic end of that range — was a good early sign that the measurement loop and grid/block sizing were right from the first CUDA kernel written, not a borderline pass.
